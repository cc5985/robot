...
price_gap_adjusted_by_volume=None

def recalculate_fees():
    '''
    this global method should be called periodically, say 1hour every epoch
    :return: nothing
    '''
    global FEES
    trading_volume=kraken.trades_volume()
    FEES=KRAKEN.Kraken.get_fees(trading_volume)

def calculate_price_gap_adjusted_by_volume():
    '''
    this global method should be called periodically, say 1min every epoch
    :return:
    '''
    global price_gap_adjusted_by_volume
    trades=kraken.trades(currency_pair)
    statistics=UNIVERSAL.Trades.statistics(trades)
    price_gap_adjusted_by_volume=statistics['avg_price_gap_adjusted_by_volume']

def recalculate_accumulated_volume_in_order_book(clean_depth):
    global price_gap_adjusted_by_volume
